...
import { buildStrapiFilters } from "./strapi-filters";
import { buildStrapiSort } from "./strapi-sort";
    ...
    adapter: ({
      ...
      return {
        ...
        update: async ({ where, update, model }) => {
          ...
          const uid = getModelUid(model);
          const filters = buildStrapiFilters(where, model, getFieldName);
        ...
        findMany: async ({ where, model, limit, offset, sortBy }) => {
          ...
          if (sortBy) {
            ...
            queryOptions.sort = buildStrapiSort(sortBy, model, getFieldName);
          }
